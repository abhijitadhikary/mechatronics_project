...
while True:
    ...
    input_image = input_image[:, 80:80 + 480]
    ...
    for i in range((len(joints_X) - 1)):
        if i == 9 or i == 5:
            continue
        cv2.putText(resized_image, "{}".format(i), (joints_X[i], joints_Y[i]), cv2.FONT_HERSHEY_PLAIN, fontScale=0.7, color=(255, 255, 255))
        start_point = (joints_X[i], joints_Y[i])
        end_point = (joints_X[i + 1], joints_Y[i + 1])
        cv2.line(resized_image, start_point, end_point, color=(255, 0, 0), thickness=2)
